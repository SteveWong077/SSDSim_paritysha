...
Status allocate_location(struct ssd_info *ssd, struct sub_request *sub_req) {
    ...
    if (ssd->parameter->allocation_scheme == 0)                                          /*��̬��������*/
    {
        ...
        if (sub_req->lpn != ssd->stripe.checkLpn && ssd->dram->map->map_entry[sub_req->lpn].state != 0) {//更新写
            if ((sub_req->state & ssd->dram->map->map_entry[sub_req->lpn].state) !=
                ssd->dram->map->map_entry[sub_req->lpn].state) {//sub_req->state为1：一对多，写到buffer；为0：不用
                ssd->read_count++;
                ssd->read1++;
...
//                }
    ...
    if ((ssd->parameter->allocation_scheme != 0) || (ssd->parameter->dynamic_allocation != 0)) {
        if (ssd->channel_head[sub_req->location->channel].subs_w_tail != NULL) {
           //printf("%u     123      %u\n", sub_req->lpn, sub_req->location->channel);
...
//                req->all++;
//                if (ssd->dram->map->map_entry[sub_req_lpn].state == 0 && ssd->parameter->allocation_scheme == 0 &&
//                    ssd->parameter->dynamic_allocation == 2) {
//                    creat_sub_write_request_for_raid(ssd, sub_req_lpn, sub_req_state, req,
//                                                     ~(0xffffffff << (ssd->parameter->subpage_page)));
//                } else {
...
//                                   size(sub_req_state);                            /*��Ϊ������ʵʱд�ز�������Ҫ������д�ز�����������*/
//            }
//        }
//
//        /******************************************************************************
...
//
//                if (hit_flag !=
//                    0)                                                          /*�����ˣ���Ҫ���ýڵ��Ƶ�buffer�Ķ��ף����ҽ����е�lsn���б��*/
//                {
...
//                        if (buffer_node ==
//                            ssd->dram->buffer->buffer_tail)                  /*������еĽڵ���buffer�����һ���ڵ㣬������������ڵ�*/
//                        {
...
//                            ssd->parameter->allocation_scheme == 0 && ssd->parameter->dynamic_allocation == 2) {
//                            creat_sub_write_request_for_raid(ssd, sub_req_lpn, sub_req_state, req,
//                                                             ~(0xffffffff << (ssd->parameter->subpage_page)));
//                        } else {
//                            //sub_req=creat_sub_request(ssd,sub_req_lpn,sub_req_size,sub_req_state,req,WRITE,0, ssd->page2Trip[sub_req_lpn]);
...
//                                }
//                                if (ssd->preSubWriteLpn[ssd->preSubWriteNowPos]) {
//                                    create_parity(ssd, req,
//                                                  ssd->page2Trip[ssd->preSubWriteLpn[ssd->preSubWriteNowPos] - 1]);
//                                }
//                            }
//                        }
//                        if (req != NULL) {
...
//                        buffer_node)                      /*�����buffer�ڵ㲻��buffer�Ķ��ף���Ҫ������ڵ��ᵽ����*/
//                    {
//                        buf_node *preNode = buffer_node->LRU_link_pre;
//                        if (ssd->dram->buffer->buffer_tail == buffer_node) {
//                            ssd->dram->buffer->buffer_tail = buffer_node->LRU_link_pre;
...
creat_sub_request(struct ssd_info *ssd, unsigned int lpn, int size, unsigned int state, struct request *req,
    ...
    unsigned int flag = 0;//%u形式输出 unsigned int
 ...
 **************************************************************************************/
    ...
    if (operation == READ) {
        ...
        sub->operation = READ;
        //设置sub->state即子页状态标志等于当前dram中lpn映射表结构中的state 即子页映射有效标志位
        ...
        if (flag == 0) {
            ...
            if (sub->current_state != SR_R_DATA_TRANSFER) {
                ...
                if (p_ch->subs_r_tail != NULL) {//判断p_ch所指的channel下的读子请求队列是否为空
                    //直接将新创建的sub插到队尾
    ...
    **************************************************************************************/
    ...
    } else {//operation既不为READ也不为WRITE，那么说明传入的operation有误，此时需要释放前面动态申请的location且释放sub，打印出错信息并返回NULL。
        free(sub->location);
        sub->location = NULL;
        free(sub);
        sub = NULL;
    ...
    if ((ssd->parameter->allocation_scheme == 0) && (ssd->parameter->dynamic_allocation == 0))    /*是完全的动态分配*/
    {
        ...
        while (sub != NULL) {
            if (sub->current_state == SR_WAIT) {
                if (sub->update != NULL)                                                      /*如果有需要提前读出的页---用update这个读请求去完成*/
                {
        ...
        if (ssd->parameter->allocation_scheme == 0)                                       /*动态分配*/
        {
            for (j = 0; j < ssd->channel_head[channel].chip; j++) {
                if ((ssd->channel_head[channel].subs_w_head == NULL) && (ssd->subs_w_head == NULL)) {//该channel上没有子请求，跳出循环，到下一个channel找
                    ...
                    //channel上有子请求，处理
                chip_token = ssd->channel_head[channel].token;//从某一个chip开始分配                            /*����*/
                /*if(ssd->parameter->dynamic_allocation == 2)
					chip_token = j;*/
...
/*********************************************************
	...
	**********************************************************/
    ...
    if (flag == 1) // don`t have read write quest
    {
        ...
        if (ssd->gc_request > 0)  //没有读写请求的时候gc                                                          /*SSD����gc����������*/
        {
            gc(ssd, 0,
               1);                                                                  /*���gcҪ������channel�����������*/
        }
    ...
    services_2_r_cmd_trans_and_complete(
            ssd);                                           //专门为读子请求服务的函数/*处理当前状态是SR_R_C_A_TRANSFER或者当前状态是SR_COMPLETE，或者下一状态是SR_COMPLETE并且下一状态预计时间小于当前状态时间*/
        ...
        if ((ssd->channel_head[i].current_state == CHANNEL_IDLE) || (ssd->channel_head[i].next_state == CHANNEL_IDLE &&
            ...
            if (ssd->gc_request > 0)                                                      /*有gc操作，需要进行一定的判断*/
            {
                if (ssd->channel_head[i].gc_command != NULL) {
                    flag_gc = gc(ssd, i,
                                 0);                                                 /*gc函数返回一个值，表示是否执行了gc操作，如果执行了gc操作(gc成功执行返回1，没有执行成功(即要先服务读写请求)返回0)，这个channel在这个时刻不能服务其他的请求*/
                }
                if (flag_gc ==
                    SUCCESS)                                                          /*ִ�й�gc��������Ҫ�����˴�ѭ��*/
                {
