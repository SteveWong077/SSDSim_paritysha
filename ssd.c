...
3，pre_process_page()函数提前扫一遍读请求，把读请求的lpn<--->ppn映射关系事先建立好，写请求的lpn<--->ppn映射关系在写的时候再建立，预处理trace防止读请求读不到数据；
            ...
            for (k = 0; k < ssd->parameter->die_chip; k++) {
                for (l = 0; l < ssd->parameter->plane_die; l++) {
                    for (m = 0; m < ssd->parameter->block_plane; m++) {
                        for (n = 0; n < 0.30 * ssd->parameter->page_block; n++) {//only 0.3
...
void raid_init(struct ssd_info *ssd) {
    ...
    for (i = 0; i < pageNum; i++)//将每个页对应的条带号初始化为-1
        ssd->page2Trip[i] = -1;


    ssd->stripe.all = 4;//3+1的条带：表示一个条带总共有4个页组成
    StripeNum = pageNum / ssd->stripe.all;//总条带数
    ssd->stripe.now = 0;//当前条带3+1中的某一位
    ssd->stripe.check = 0;//?
    ...
    ssd->stripe.allStripe = pageNum / ssd->stripe.all + 1;//总条带数
...
void add_invalid_date(struct ssd_info *ssd) {
    ...
    ssd->allPage = ((ssd->parameter->page_block) * (ssd->parameter->block_plane) * (ssd->parameter->plane_die) *
                    ...
                    (ssd->parameter->channel_number * 3 / 4));//已改
    int channToken = ssd->channelToken;
    int chipToken = ssd->chipToken;
    int ssdToken = ssd->ssdToken;
    while (ssd->allUsedPage < ssd->allPage * 0.7) {//不断预填，超过设定的阈值，find_active_block()报错
...
//	 abort();
    ...
    //printf("ssd->dram->map->map_entry[lpn].state:%u\n",ssd->dram->map->map_entry->state);
    //pre_process_page(ssd);
    // get_old_zwh(ssd);
    // add_valid_date(ssd);
    add_invalid_date(ssd);
    init_ppc_cache(ssd);//ppc_cache---first!
...
int get_requests(struct ssd_info *ssd) {
    ...
    filepoint = ftell(ssd->tracefile);//返回文件位置
    fgets(buffer, 200, ssd->tracefile);//用于在指定流中获取一行字符，并存储在一个字符数组中
    if (strlen(buffer) <= 1) {
        return 0;
    }
    sscanf(buffer, "%lld %d %d %d %d %d", &time_t, &device, &lsn, &size, &ope, &priority);//从字符串中读取格式化输入
	...
	***********************************************************************************************************/
    ...
    request1 = (struct request *) malloc(sizeof(struct request));//创建请求--分配内存并初始化请求结构体
    ...
    if (ssd->request_queue == NULL)          //The queue is empty--更新请求队列
    {
...
void trace_output(struct ssd_info *ssd) {
    ...
    struct sub_request *sub, *tmp, *sub_req = NULL;

    unsigned int full_page, page_count, free_page = 0;//已改
    unsigned int sub_req_state = 0, sub_req_size = 0, sub_req_lpn = 0;//已改
    struct buffer_group *buffer_node = NULL, *new_node = NULL, *pt = NULL,key;//已改
#ifdef DEBUG
        ...
        if (req->response_time != 0) {
            ...
            if(ssd->parameter->dram_capacity!=0){//判断是否有缓存
                while(sub!=NULL && req->operation == READ){//---这里判断req的类型，而不是判断sub类型，因为
                    //printf("11111\n");
                    if(((sub->current_state == SR_COMPLETE)||((sub->next_state==SR_COMPLETE)&&(sub->next_state_predict_time<=ssd->current_time))) && sub->state!=0)	// if any sub-request is not completed, the request is not completed
                    {
                        ssd = insert2buffer(ssd,sub->lpn,sub,req);
                        //ssd->avg_read_times++;
//                        if (ssd->dram->map->map_entry[sub->lpn].read_flag == 0)
//                        {
//                            ssd->dram->map->map_entry[sub->lpn].read_flag = 1;
//                            ssd->read_page_num++;
//                        }
                    }
                    sub = sub->next_subs;
            ...
            while (sub != NULL) {
                ...
                {
//                    if (sub->operation == READ && sub->lpn != ssd->stripe.checkLpn) {
//                        free_page = ssd->dram->databuffer->max_buffer_page - ssd->dram->databuffer->current_buffer_page;
//                        //printf("free_page of rw cache:%d\n",free_page);
//                        key.group=sub->lpn;
//                        new_node = NULL;
...
//                        new_node->group = sub->lpn;//是否正确---大请求的lpn和子请求的lpn是否一样
//                        new_node->stored = sub->state;//?---判断扇区是否在缓存里面，1在，0不在
//                        new_node->dirty_clean = 0;//----判断子页是否被更新
//
//                        new_node->LRU_link_pre = NULL;
//                        new_node->LRU_link_next = NULL;
//
//                        buffer_node = (struct buffer_group *) hash_find(ssd->dram->databuffer, (HASH_NODE *) &key);//参数是否有误
//                        if (buffer_node != NULL) {
//                            //printf("hhhhhh\n");//说明重复插入---与
//                            //while(1);//一直停在这
//                            //new_node->stored=sub->state | buffer_node->stored ;
//                            buffer_node->stored = buffer_node->stored | sub->state;//扇区进行管理，是否与页冲突
//                            if (new_node != NULL) {
//                                free(new_node); // free(req/sub)
//                                new_node = NULL;
//                            }
//                        } else {
//                            //判断buffer是否满
//                            if (free_page >= 1) {//以页为单位管理，为 1 即可
//                                if (ssd->dram->databuffer->buffer_head != NULL) {   //双向链表
//                                    new_node->LRU_link_next = ssd->dram->databuffer->buffer_head;
//                                    ssd->dram->databuffer->buffer_head->LRU_link_pre = new_node;
//                                } else {
//                                    ssd->dram->databuffer->buffer_tail = new_node;
//                                }
//                                ssd->dram->databuffer->buffer_head = new_node;
//                                hash_add(ssd->dram->databuffer, (HASH_NODE *) new_node);
//                                ssd->dram->databuffer->current_buffer_page++;//已改:以页为单位，+1 即可
//                            } else {
//                                pt = ssd->dram->databuffer->buffer_tail;
...
//                                    } else {
//                                        creat_sub_request(ssd, sub_req_lpn, sub_req_size, sub_req_state, req,
//                                                          WRITE, 0,
...
//                                hash_del(ssd->dram->databuffer, (HASH_NODE *) pt);
//                                hash_node_free(ssd->dram->databuffer, (HASH_NODE *) pt);
//                                pt = NULL;
//                                ssd->dram->databuffer->current_buffer_page--;//以页为单位，-1 即可
//
//                                //move_to_phead();
...
void free_all_node(struct ssd_info *ssd) {
    ...
    hash_destroy(ssd->dram->databuffer);//已改---释放缓存
...
void creat_sub_write_request_for_raid(struct ssd_info *ssd, int lpn, unsigned int state, struct request *req,
    ...
    if (ssd->stripe.now == 0) {
        ...
        ssd->trip2Page[ssd->stripe.nowStripe].Pchannel =
                (ssd->ssdToken) * ssd->perChanSSD + ssd->channelToken;//已改：ssd->stripe.check
        if (ssd->trip2Page[ssd->stripe.nowStripe].Pchannel >= 16) {//已改
...
struct ssd_info *no_buffer_distribute(struct ssd_info *ssd) {
    ...
    if (req->operation == READ) {
        while (lpn <= last_lpn) {
            ...
            lpn += 1;
            if(find_node_READ(ssd,lpn,req)==0){
...
//                                            ssd->page2Trip[lpn]);
//                    //printf("ssd->page2Trip[lpn]:%u----lpn:%u\n",ssd->page2Trip[lpn],lpn);前者lpn对应的条带号，后者是lpn
//                    ppc_cache(ssd, ssd->page2Trip[lpn], state, req, mask, lpn, sub);
//                }
//            }
                find_node(ssd,lpn,req);
//            if(ssd->dram->map->map_entry[lpn].state != 0){
//                ppc_cache(ssd, ssd->page2Trip[lpn], state, req, mask, lpn, sub);//这句加在哪里
//            }
...
struct ssd_info *no_buffer_distribute_backup(struct ssd_info *ssd) {
