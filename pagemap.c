...
struct ssd_info *pre_process_page(struct ssd_info *ssd) {
    ...
    if (ssd->parameter->allocation_scheme == 0 && ssd->parameter->dynamic_allocation == 2)
        largest_lsn = (unsigned int) (
                ((ssd->parameter->chip_num - 1) * ssd->parameter->die_chip * ssd->parameter->plane_die *
                 ssd->parameter->block_plane * ssd->parameter->page_block * ssd->parameter->subpage_page) *
    ...
    /*计算出这个ssd的最大逻辑扇区号*/
    while (fgets(buffer_request, 200, ssd->tracefile)) {//逐行从tracefile文件中每次读取199字符，直到读完整个trace为止
        ...
        if (ope == 1)                                                      /*这里只是读请求的预处理，需要提前将相应位置的信息进行相应修改*/
        {
            while (add_size < size)//若已经预处理的操作长度<操作的长度，则执行下面操作
            {
                ...
                if (add_size + sub_size >=
                    size)                             /*ֻ�е�һ������Ĵ�СС��һ��page�Ĵ�Сʱ�����Ǵ���һ����������һ��pageʱ������������*/
                {
            ...
            ********************************************************************************************************/
                ...
                if (ssd->dram->map->map_entry[lpn].state == 0)                /*状态为0的情况*/
                {
                    ...
                    ***************************************************************/
                    ...
                    ssd->dram->map->map_entry[lpn].state = set_entry_state(ssd, lsn,
                                                                           sub_size);   //0001 （获取的子请求状态赋值给逻辑页）
                ...
                {
                    map_entry_new = set_entry_state(ssd, lsn, sub_size);       /*得到新的状态，并与原来的状态 相或 的到一个状态*/
                    map_entry_old = ssd->dram->map->map_entry[lpn].state;
                    modify = map_entry_new | map_entry_old; //相或得到一个新状态
...
void raid_pre_invalid(struct ssd_info *ssd) {
    ...
    ppn = get_ppn_for_pre_process(ssd, 0);//注:lsn==0
    location = find_location(ssd, ppn);
    ssd->program_count++;
    ssd->channel_head[location->channel].program_count++;
    ssd->channel_head[location->channel].chip_head[location->chip].program_count++;
    ...
    ++ssd->allUsedPage;//已经预填了多少页
    free(location);
    location = NULL;
}

void
raid_pre_read(struct ssd_info *ssd, unsigned int lsn, unsigned int lpn, unsigned int sub_size, unsigned int full_page) {
    unsigned int ppn;
    struct local *location;
    ...
    ssd->dram->map->map_entry[lpn].lpnFlag = 0;//已改---
    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn = lpn;
    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].valid_state = ssd->dram->map->map_entry[lpn].state;
    ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].free_state = (
            (~ssd->dram->map->map_entry[lpn].state) & full_page);
...
void init_ppc_cache(struct ssd_info *ssd) {
    ...
    if (ssd->stripe.now != 0) {
        ssd->trip2Page[ssd->stripe.nowStripe].Pchannel =
                (ssd->ssdToken + ssd->stripe.check) * ssd->perChanSSD + ssd->channelToken;
...
//        }
        ...
        for (i = 0; i < ssd->stripe.now; i++) {
            ...
            raid_pre_read(ssd, ssd->stripe.req[i].lpn * ssd->parameter->subpage_page, ssd->stripe.req[i].lpn,
                          ssd->stripe.req[i].sub_size, ssd->stripe.req[i].full_page);
...
struct ssd_info *pre_process_page_raid_1(struct ssd_info *ssd) {
    ...
    while (fgets(buffer_request, 200, ssd->tracefile)) {
        sscanf(buffer_request, "%lld %d %d %d %d", &time, &device, &lsn, &size, &ope);
        fl++;
        trace_assert(time, device, lsn, size,
                     ope);                         /*���ԣ���������time��device��lsn��size��ope���Ϸ�ʱ�ͻᴦ��*/
        ssd->total_request_num++;
        add_size = 0;                                                     /*add_size����������Ѿ�Ԥ�����Ĵ�С*/

        if (ope ==
            1)                                                      /*����ֻ�Ƕ������Ԥ��������Ҫ��ǰ����Ӧλ�õ���Ϣ������Ӧ�޸�*/
        {
            while (add_size < size) {
                ...
                if ((sub_size > ssd->parameter->subpage_page) ||
                    (add_size > size))/*��Ԥ����һ���Ӵ�Сʱ�������С����һ��page�����Ѿ������Ĵ�С����size�ͱ���*/
                {
                    printf("pre_process sub_size:%d\n", sub_size);
                }
                    ...
                    if (++ssd->stripe.now == (ssd->stripe.all - 1)) {
                        ...
                        for (i = 0, j = 0; i < ssd->stripe.all; ++i) {
                            if (i == ssd->stripe.check) {
                                raid_pre_read(ssd, ssd->stripe.checkLpn * ssd->parameter->subpage_page,
                                              ssd->stripe.checkLpn, mask, full_page);
                            ...
                            raid_pre_read(ssd, ssd->stripe.req[j].lpn * ssd->parameter->subpage_page,
                                          ssd->stripe.req[j].lpn, ssd->stripe.req[j].sub_size,
                                          ssd->stripe.req[j].full_page);
...
struct ssd_info *pre_process_page_raid(struct ssd_info *ssd) {
    ...
    while (fgets(buffer_request, 200, ssd->tracefile)) {
        ...
        {
            lpn = lsn / ssd->parameter->subpage_page;
            unsigned long long last_lpn =
                    (lsn + size + ssd->parameter->subpage_page - 1) / ssd->parameter->subpage_page;
...
unsigned int get_ppn_for_pre_process(struct ssd_info *ssd, unsigned int lsn) {
    ...
    lpn = lsn / ssd->parameter->subpage_page;//lsn传的0，所以lpn为0
    ...
    if (ssd->parameter->allocation_scheme == 0)                           /*��̬��ʽ�»�ȡppn*/
    {
                   ...
                   1)                 /*��ʾ�붯̬��ʽ��channel��̬������package��die��plane��̬����*/
        {
            ...
            ssd->channel_head[channel].chip_head[chip].die_head[die].token = (plane + 1) % ssd->parameter->plane_die;
            if (++ssd->ssdToken >=
                (ssd->parameter->channel_number / ssd->perChanSSD)) {//加到最后面---ssdTOKEN自加 1--填补未初始化为1的情况
...
get_ppn(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane,
	...
	*/
    ...
    //printf("lpn:state:%u,%u\n",lpn,ssd->dram->map->map_entry[lpn].state);
    ...
    if (lpn == ssd->stripe.checkLpn) {
        ...
        ssd->dram->map->map_entry[lpn].pn = find_ppn(ssd, channel, chip, die, plane, block, page);
               ...
               0)        //没有映射关系，表示该子请求不是更新请求，可以直接写入               /*this is the first logical page*/
    {
...
Status
fast_erase_planes(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die1, unsigned int plane1,
                  unsigned int command) {
    unsigned int die = 0;
    unsigned int plane = 0;
    unsigned int block = 0;
            ...
            if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].lpn ==
                ...
                if (((struct buffer_group *) hash_find(ssd->dram->buffer, (HASH_NODE *) &key)) != NULL) {
                    ...
                    if (ssd->trip2Page[raidid].location->channel != channel ||
                        ssd->trip2Page[raidid].location->chip != chip || ssd->trip2Page[raidid].location->die != die ||
                        ssd->trip2Page[raidid].location->plane != plane ||
                        ssd->trip2Page[raidid].location->block != block || ssd->trip2Page[raidid].location->page != i) {
                        abort();
                    }
                    free(ssd->trip2Page[raidid].location);
                    ssd->trip2Page[raidid].location = NULL;
...
int
uninterrupt_fast_gc(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane,
...
int
uninterrupt_dr(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, unsigned int plane) {
    unsigned int i = 0, invalid_page = 0;
    unsigned int block, active_block, transfer_size, free_page, page_move_count = 0;                           /*��¼ʧЧҳ���Ŀ��*/
    struct local *location = NULL;
    unsigned int total_invalid_page_num = 0;
...
int decide_gc_invoke(struct ssd_info *ssd, unsigned int channel) {
    ...
    if ((ssd->channel_head[channel].subs_r_head == NULL) &&
        (ssd->channel_head[channel].subs_w_head == NULL))    /*������Ҷ�д�������Ƿ���Ҫռ�����channel�����õĻ�����ִ��GC����*/
    {
